...
| App | Entry script | Purpose |
|---|---|---|
| `ppt_worker` | `scripts/ppt_worker.py` | Generates `.pptx` worship slides from text content files |
| `bible_reader` | `scripts/bible_reader_gui.py` | Chinese/English Bible reader with search (Whoosh + Jieba), media playback |
| `py_scheduler` | `bin/manager_gui.py` | PyQt5 GUI for MongoDB — schedules worship roles, tracks finances, library, personnel |
| `bulletin_worker` | `scripts/bulletin_worker.py` | Generates monthly church bulletin as a `.docx` Word document |
...
`PyQt5`, `pymongo`, `python-pptx`, `python-docx`, `jieba`, `whoosh`, `yt-dlp`, `pypinyin`, `qdarkstyle`, `bcrypt`, `python-dotenv`
